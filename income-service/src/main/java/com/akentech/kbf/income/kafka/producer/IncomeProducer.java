...
public class IncomeProducer {
    ...
    public Mono<Void> sendIncome(Income income) {
        return Mono.fromCallable(() -> {
                    CompletableFuture<SendResult<String, Income>> future =
                            kafkaTemplate.send("income-processing-topic", income);

                    return future.handle((result, ex) -> {
                        if (ex != null) {
                            log.error("Failed to send income to Kafka: {}", ex.getMessage());
                            throw new RuntimeException("Kafka send failed", ex);
                        }
                        log.info("Sent income to Kafka. Offset: {}", result.getRecordMetadata().offset());
                        return result;
                    });
                })
